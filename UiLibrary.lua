...
local function SafeCallback(callback, ...)
    local args = {...}
    local success, result = pcall(function() return callback(table.unpack(args)) end)
    if not success then
        local errMsg = tostring(result)
        if typeof(setclipboard) == "function" then
            pcall(setclipboard, errMsg)
        end
        warn("[UILibrary ERROR] " .. errMsg)
        if UILibrary.Notify then
            UILibrary.Notify("Callback Error", errMsg, 7)
        end
    end
    return result
end
...
-- Tab metatable
UILibrary.Tab = {}
UILibrary.Tab.__index = UILibrary.Tab
...
function UILibrary.CreateWindow(title)
    local window = setmetatable({}, UILibrary.Window)
...
function UILibrary.Window:AddTab(name)
    local tab = setmetatable({}, UILibrary.Tab)
...
function UILibrary.Tab:AddSection(title)
    local section = setmetatable({}, UILibrary.Section)
...
function UILibrary.Section:AddSlider(text, min, max, default, callback, decimal)
    ...
    local function updateSlider(input)
        local relX = (input.Position.X - slider.Track.AbsolutePosition.X) / slider.Track.AbsoluteSize.X
...
function UILibrary.Section:AddDropdown(text, options, default, callback)
    ...
    dropdown.Button.MouseButton1Click:Connect(function() dropdown:Toggle() end)
    dropdown.Button.TouchTap:Connect(function() dropdown:Toggle() end)
